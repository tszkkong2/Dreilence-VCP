...
function BeginPath(x, y) {
	//Chat.socket.send('1,'+x+','+y);
    var message = ('1,'+x+','+y);
    saveToClient(message);
    socket.emit('drawmsg',message);
...
function clear_all(){
	...
	//Chat.socket.send('clearAll');
    socket.emit('drawmsg','clearAll');
}
...
// 	Chat.socket.onmessage = function (n) {
// 		var res = (n.data).split(",");
// 		var e;
// 		if (canDraw)
// 			if(n.data == 'clearAll')
// 				clearBoard();
// 			else if (res[0] == 1){
// 				e = new BeginPath1(res[1], res[2])

// 			}else if (res[0] == 2)
// 				e = new ClosePath1()
// 			else if (res[0] == 3)
// 				e = new DrawPathToPoint1(res[1], res[2])
...
// 	Chat.connect('ws://myfyp1.no-ip.org:8080/examples/websocket/chat');
// };
...
socket.on('setId',function(message){
    client.setSocket(message);
});

function saveToClient(msg){
    client.curseq++;
    var cseq = client.curseq;
    client.sequences.push("Seq " + cseq + ". " + msg);

    vcChecker.increment(client.clocks,'node-1');

    console.log(client);
}

var test = 
socket.on('drawmsg',function(message){
